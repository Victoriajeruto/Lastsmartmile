...
export class NotificationService {
  ...
  static async notifyDeliveryAssigned(
    ...
    if (user?.phone) {
      await smsService.sendDeliveryAssignedNotification(
        user.phone,
        trackingNumber,
        boxId,
      );
  ...
  static async notifyDeliveryDelivered(
    recipientId: string,
    trackingNumber: string,
    boxId: string,
  ): Promise<void> {
    ...
    const user = await storage.getUser(recipientId);
    if (user?.phone) {
      await smsService.sendDeliveryDeliveredNotification(
        user.phone,
        trackingNumber,
        boxId,
      );
  ...
  static async notifyTamperDetected(
    userId: string,
    boxId: string,
  ): Promise<void> {
    await this.createNotification({
  ...
  static async sendUnlockCodeSMS(
    userId: string,
    code: string,
    boxId: string,
  ): Promise<void> {
