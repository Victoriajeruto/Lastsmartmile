...
export async function registerRoutes(app: Express) {
  ...
  app.post(
    ...
    async (req: AuthenticatedRequest, res) => {
      try {
        ...
        res
          .status(400)
          .json({ message: error.message || "Box registration failed" });
      }
    },
  );
  ...
  app.get("/api/boxes", requireAuth, async (req: AuthenticatedRequest, res) => {
    try {
      ...
      res
        .status(500)
        .json({ message: error.message || "Failed to fetch boxes" });
    }
  });
  ...
  app.get(
    ...
    async (req: AuthenticatedRequest, res) => {
      try {
        ...
        res
          .status(500)
          .json({ message: error.message || "Failed to fetch box" });
      }
    },
  );
  ...
  // Unlock box routes
  app.post(
    "/api/boxes/:boxId/unlock/generate",
    requireAuth,
    async (req: AuthenticatedRequest, res) => {
      try {
        ...
        const unlockCode = await storage.createUnlockCode({
          ...
          expiresAt:
            method === "otp"
              ? OTPService.getExpirationTime()
              : QRService.getExpirationTime(),
          isUsed: false,
        });

        if (method === "otp" && otpCode) {
          await NotificationService.sendUnlockCodeSMS(
            req.user!.id,
            otpCode,
            boxId,
          );
        ...
        res
          .status(500)
          .json({ message: error.message || "Failed to generate unlock code" });
      }
    },
  );
  ...
  app.post("/api/boxes/:boxId/unlock/verify", async (req, res) => {
    try {
      ...
      if (method === "otp" && activeUnlockCode.otpCode) {
        ...
        isValid =
          activeUnlockCode.qrCode === code &&
          QRService.verifyQRCode(code, boxId);
      }
      ...
      // Send notification
      await NotificationService.notifyBoxUnlocked(
        activeUnlockCode.userId,
        boxId,
      );
      ...
      res
        .status(500)
        .json({ message: error.message || "Failed to unlock box" });
    }
  });
  ...
  app.post(
    ...
    async (req: AuthenticatedRequest, res) => {
      try {
        ...
        res
          .status(400)
          .json({ message: error.message || "Failed to assign delivery" });
      }
    },
  );
  ...
  app.patch(
    "/api/deliveries/:id/assign-to-me",
    requireAuth,
    requireRole(["courier"]),
    async (req: AuthenticatedRequest, res) => {
      try {
        ...
        if (delivery.status !== "pending") {
          return res
            .status(400)
            .json({ message: "Delivery is not available for assignment" });
        }
    ...
    async (req: AuthenticatedRequest, res) => {
      try {
        ...
        if (req.user!.role === "admin") {
          ...
          const assignedDeliveries = await storage.getDeliveriesByCourierId(
            req.user!.id,
          );
          ...
          [...assignedDeliveries, ...pendingDeliveries].forEach((d) =>
        ...
        res
          .status(500)
          .json({ message: error.message || "Failed to fetch deliveries" });
      }
    },
  );

  app.get(
    "/api/deliveries/route/optimized",
    requireAuth,
    requireRole(["courier"]),
    async (req: AuthenticatedRequest, res) => {
      try {
        ...
        const validLocations = deliveryLocations.filter(
        ...
        res
          .status(500)
          .json({ message: error.message || "Failed to optimize route" });
      }
    },
  );
  ...
  app.get("/api/iot/boxes/:boxId/status", async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ message: error.message || "Failed to get box status" });
    }
  });
  ...
  app.post("/api/iot/boxes/:boxId/status", async (req, res) => {
    try {
      ...
      if (
        ...
        await NotificationService.notifyLowBattery(
          box.ownerId,
          boxId,
          batteryLevel,
        );
      ...
      res
        .status(500)
        .json({ message: error.message || "Failed to update box status" });
    }
  });
  ...
  app.get(
    ...
    async (req: AuthenticatedRequest, res) => {
      try {
        ...
        res
          .status(500)
          .json({ message: error.message || "Failed to get subscriptions" });
      }
    },
  );
  ...
  app.get(
    ...
    async (req: AuthenticatedRequest, res) => {
      try {
        ...
        res
          .status(500)
          .json({ message: error.message || "Failed to get user count" });
      }
    },
  );
      ...
      try {
        ...
        const tamperStats = {
          ...
          last24Hours: allEvents.filter((e) => {
            const hoursDiff =
              (Date.now() - new Date(e.detectedAt).getTime()) /
              (1000 * 60 * 60);
            return hoursDiff <= 24;
          }).length,
  ...
  app.get(
    "/api/analytics/boxes/battery",
    requireAuth,
    requireRole(["admin"]),
    async (req: AuthenticatedRequest, res) => {
      try {
        const boxes = await storage.getAllBoxes();
        const boxesWithBattery = boxes.filter(
          (b) => b.batteryLevel !== null && b.batteryLevel !== undefined,
        );
        const boxesWithoutBattery = boxes.filter(
          (b) => b.batteryLevel === null || b.batteryLevel === undefined,
        );
        ...
        const batteryStats = {
          ...
          low: boxesWithBattery.filter(
            (b) => b.batteryLevel! >= 10 && b.batteryLevel! < 20,
          ).length,
          medium: boxesWithBattery.filter(
            (b) => b.batteryLevel! >= 20 && b.batteryLevel! < 50,
          ).length,
          good: boxesWithBattery.filter((b) => b.batteryLevel! >= 50).length,
          averageBatteryLevel:
            boxesWithBattery.length > 0
              ? boxesWithBattery.reduce((sum, b) => sum + b.batteryLevel!, 0) /
                boxesWithBattery.length
              : 0,
    ...
    async (req: AuthenticatedRequest, res) => {
      try {
        ...
        const payment = await storage.createPayment({
          ...
          description:
            description ||
            `Payment for Last Mile Postal System - ${paymentType || "subscription"}`,
        });
        ...
        try {
          ...
          res.json({
            message:
              "Payment initialized. Please complete payment on the provided URL.",
            payment: {
              id: payment.id,
              amount: amount,
              reference,
  ...
  app.post("/api/payments/webhook", async (req, res) => {
    try {
      ...
      if (event.event === "charge.success") {
        ...
        console.log(
          `✅ Payment completed - Reference: ${reference}, Amount: KES ${amount / 100}, Channel: ${channel}, Plan: ${subscriptionPlan}`,
        );
    ...
    async (req: AuthenticatedRequest, res) => {
      try {
        ...
        if (payment.status === "pending" && payment.reference) {
          try {
            const verifyResponse = await paystackService.verifyTransaction(
              payment.reference,
            );
        ...
        res
          .status(500)
          .json({ message: error.message || "Failed to check payment status" });
      }
    },
  );
  ...
  app.post("/api/installation-requests", async (req, res) => {
    try {
      ...
      const installationRequest =
        await storage.createInstallationRequest(requestData);
  ...
  app.get(
    "/api/installation-requests/pending",
    requireAuth,
    requireRole(["admin"]),
    async (req: AuthenticatedRequest, res) => {
      try {
    ...
    async (req: AuthenticatedRequest, res) => {
      try {
        ...
        if (!request) {
          return res
            .status(404)
            .json({ message: "Installation request not found" });
        }
  ...
  app.get("/api/service-pricing", async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ message: error.message || "Failed to fetch service pricing" });
    }
  });

  app.get(
    "/api/service-pricing/all",
    requireAuth,
    requireRole(["admin"]),
    async (req: AuthenticatedRequest, res) => {
      try {
  ...
  app.get(
    ...
    async (req: AuthenticatedRequest, res) => {
      try {
        ...
        res
          .status(500)
          .json({ message: error.message || "Failed to fetch active boxes" });
      }
    },
  );
